...
if alive; then
  echo "Robinhood Radar V1.2.5 已运行，PID=$(cat "$PIDFILE")"
else
  ...
  echo "✅ Robinhood Radar V1.2.5 已启动，PID=$(cat "$PIDFILE")"
fi

# Optional Android JobScheduler safety net (Termux:API required).
...
echo "状态：bash status-termux.sh"
echo "请按 Home 键最小化 Termux；不要划掉最近任务，也不要“强制停止”。"
