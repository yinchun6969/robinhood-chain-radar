...
echo "[3/6] Copy V1.3.0 files / 更新文件"
FILES=(monitor.py address_intel.py token_intel.py token_radar.py token_worker.py dashboard.py fast_scanner.py event_worker.py swap_filter.py v4_resolver.py native_scanner.py radar_supervisor.py start-termux.sh stop-termux.sh status-termux.sh enable-boot.sh disable-boot.sh keepalive-job.sh requirements.txt test_static.py test_integration.py)
...
# Importing monitor initializes all backward-compatible V1.3 SQLite tables.
.venv/bin/python - <<'PY'
import monitor
print('DB schema V1.3.0: OK')
PY

echo "[5/6] RPC self-test"
.venv/bin/python monitor.py --self-test

echo "[6/6] Start V1.3.0"
